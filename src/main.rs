...
#[tokio::main]
async fn main() {
    // Initialize the application. This also pins the process-wide path root
    // (`syscity::init`), so the CLI's single-root contract is established before
    // any `dirs::` free function runs.
